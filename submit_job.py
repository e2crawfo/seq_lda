...
def main(task, n_jobs, input_zip, n_nodes=1, ppn=12, walltime="1:00:00", add_date=False, test=0, scratch=None, verbose=0):
...
module load python/2.7.2'''
    ...
    if test:
        command = '''
seq 0 $(({n_jobs}-1)) | parallel --eta --no-notice -j{n_procs} --workdir $PWD sd-experiment {task} {{}} --d {local_scratch}/{name}_{task} --verbose {verbose}
'''
    else:
        command = '''
seq 0 $(({n_jobs}-1)) | parallel --eta --no-notice -j{n_procs} --sshloginfile $PBS_NODEFILE --workdir $PWD sd-experiment {task} {{}} --d {local_scratch}/{name_{task} --verbose {verbose}
''' # noqa
...
mpiexec -np {n_nodes} -pernode sh -c 'cp {input_zip} {local_scratch} && \\
                                      unzip {local_scratch}/{input_zip_bn} -d {local_scratch} && \\
                                      mv {local_scratch}/{name} {local_scratch}/{name}_{task}'

# START PARALLEL JOBS USING NODE LIST IN $PBS_NODEFILE
''' + command + '''
cd {local_scratch}
mpiexec -np {n_nodes} -pernode sh -c 'zip -r $OMPI_COMM_WORLD_RANK {name}_{task} && \\
